...
template <class T>
void Vec<T>::create(ConstIterator b, ConstIterator e)
{
...
typename Vec<T>::Iterator Vec<T>::erase(Iterator b, Iterator e)
{
    if (b == e)
        return b;
    Vec temp;
    temp.create(e, avail_);
    for (Iterator i = b; i != avail_; ++i)
        alloc_.destroy(i);
    avail_ = std::uninitialized_copy(temp.begin(), temp.end(), b);
    return b;
}
