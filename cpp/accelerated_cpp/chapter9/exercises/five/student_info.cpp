...
char StudentInfo::letterGrade() const
{
    double grade = (midterm_ + finalterm_)/2;
    if (grade < 60)
        return 'F';
    else return 'P';
}
...
bool StudentInfo::isValid() const
{
...
double StudentInfo::grade() const
{
    ...
    else throw domain_error("There are no homework grades.");
}
